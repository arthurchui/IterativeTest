...
namespace NUnitExtension.IterativeTest.AddIn
{
    public class IterativeTestNameBuilder
    {
        ...
        public IterativeTestNameBuilder(MethodInfo method, string baseTestName, object[] arguments)
        {
            _method = method;
        ...
        private string GetArgumentString(object argument)
        {
            ...
            PropertyInfo pi = argument.GetType().GetProperty("Id", BindingFlags.Instance | BindingFlags.Public);
            if (pi != null)
            {
                return pi.GetValue(argument, null).ToString();
            }
